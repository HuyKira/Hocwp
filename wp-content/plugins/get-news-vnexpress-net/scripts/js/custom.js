...
jQuery(document).ready(function() {
    ...
    jQuery('.click-more-check').click(function(event) {
        var url = jQuery('#linkchuyenmuc').val();
        jQuery.ajax({
            type: 'POST',
            ...
            success: function (data) {
                jQuery('.list-input').html('');
                for (var i = 0; i < data.length; i++) {
                    var html = '<div class="form-group">';
                        html +='<label for="link">Nhập link bài viết</label>';
                        html +='<input required="required" name="link[]" value="'+data[i]+'" type="url" class="form-control" id="" placeholder="Nhập link vào đây">';
                        html +='<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>';
                        html +='</div>';
                    jQuery('.list-input').append(html);
                }
